...
class Info(commands.Cog):
    ...
    def calculatetime(
        self, age
    ):  # calculate time and mention how long has it been in a human readable format
    ...
    async def userinfo_error(self, ctx: commands.Context, error):
        if isinstance(error, commands.RoleNotFound):
            ...
            embed.set_footer(
                text="because toby is too lazy to configure this command to be used in every realm server"
            )
            await ctx.send(embed=embed)
        else:
    ...
    async def serverinfo(self, ctx: commands.Context):
        ...
        for channel in server.channels:
            if (
                ...
                or channel.type == discord.ChannelType.news
            ):
                txt += 1
            elif (
                channel.type == discord.ChannelType.voice
                or channel.type == discord.ChannelType.stage_voice
            ):
                voice += 1
        ...
        embed.add_field(
            name="Owner",
            value=f"{server.owner.name}#{server.owner.discriminator}, ID: {server.owner.id}",
            inline=True,
        )
        embed.add_field(
            name=f"{len(server.channels)} channels",
            value=f"Text: {txt}\nVoice: {voice}\nCategories: {category}",
            inline=True,
        )
    ...
    async def categoryinfo(self, ctx: commands.Context, cat: discord.CategoryChannel):
        ...
        if len(cat.text_channels) < 50:
            embed.add_field(
                name=f"{len(cat.text_channels)} text channels",
                value="category not full",
                inline=False,
            )
        else:
            embed.add_field(
